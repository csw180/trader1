    ...
    while current_time < nextday :
        try:
            ...
            if start_time < current_time < end_time :
                current_price = get_current_ask_price(current_ticker)
                ...
                if target_price < current_price:
                    ...
                    if krw > 5000:
                        ret = upbit.buy_limit_order(current_ticker, current_price, (krw*0.9995)//current_price )
                        print(f'buy_limit_order {current_ticker}, {krw*0.9995:.2f}', flush=True)
                        print(f'buy_limit_order ret = {ret}' , flush=True)
            else:
                btc = get_balance(currency)
                print(f'get_balance(BTC): {btc}', flush=True)
                current_price = get_current_bid_price(current_ticker)
                if btc > ( 5000 / current_price) :
                    ret = upbit.sell_limit_order(current_ticker, current_price, (btc*0.9995)//current_price )
                    print(f'sell_limit_order {current_ticker}, {btc*0.9995:.2f}', flush=True)
